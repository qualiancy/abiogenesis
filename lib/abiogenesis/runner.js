...
Runner.prototype.runDefinition = function (type, name, cb) {
  cb = cb || function () {};
  var base = getSpec(this._definitions, type, name);

  if (!base) {
    return cb(new Error('Definition `' + name + '` of type `' + type + '` does not exist.'));
  }
...
Runner.prototype.runContext = function (type, name, cb) {
  ...
  if (!base) {
    return cb(new Error('Context `' + name + '` of type `' + type + '` does not exist.'));
  }
