...
Runner.prototype.useDefinition = function (def, action) {
...
Runner.prototype.addDefinition = function (type, name) {
  var self = this
    , definition;

  function checkValid (_type, _name) {
    var key = getKey(_type, _name);
    if (!self._types[_type])
      throw new Error('no definition for type `' + _type + '`');
    if (getSpec(self._definitions, key))
      throw new Error(_type + ' named `' + _name + '` already defined');
  }
...
Runner.prototype.addContext = function (type, name) {
  var self = this;
  ...
  if ('object' === typeof type) {
    var context = type;
    type = context._opts.type;
    name = context._opts.name;
    checkValid(type, name);
  } else {
  ...
  this.emit([ 'context', 'add', type ], type, context);
};

Runner.prototype.pushContext = function (context) {
  var self = this;

  function iterate (ctx) {
    self.addContext(ctx);
    for (var d = 0, ld = ctx._definitions.length; d < ld; d++)
      self.addDefinition(ctx._definitions[d]);
    for (var c = 0, lc = ctx._contexts.length; c < lc; c++)
      iterate(ctx._contexts[c]);
  }
...
Runner.prototype.runContext = function (type, name, cb) {
  ...
  function locateDefs (context) {
    context._definitions.forEach(function (def) {
      defs.push(def);
    });
    context._contexts.forEach(function (ctx) {
      locateDefts(ctx);
    });
  }
